...
#
# Writes: dist/armoredcoremasterofarena-<VERSION>-<artifact-tag>.zip
set -euo pipefail
...
exec bash "${PACKAGER}" \
  ...
  --zip-prefix armoredcoremasterofarena \
  --exe-name Armored_Core_Master_of_Arena \
  --omit-openbios \
  --display-name "Armored Core: Master of Arena" \
  --recompiler-build "${RECOMPILER_BUILD}" \
  --version-env RELEASE_VERSION \
  --disc-hint "your legally owned Armored Core: Master of Arena (USA) discs" \
  --runtime-dir mods \
